...
`com.gthbj.tracking`：埋点公共库。消费者是 `boopfun-limited/arrows`、`boopfun-limited/water_sort`、`boopfun-limited/arrows-3d`（2026-09-19 接入）、`boopfun-limited/ball-sort`、`boopfun-limited/boopdoku` 与 `boopfun-limited/sudoku`，都按 40 位 commit SHA 钉本仓（名单以各仓 `Packages/manifest.json` 里有没有本包为准）。
