...
只有 Firebase / MAX 收得到拒绝）在数据与个性化上**只增不减**，在合规上弱于我们自己的政策承诺。
owner 在看过逐条方向后选择与原包一致，见 §10。
...
- **收得到**：Firebase（UMP 自己经反射写）、MAX（自带 TcfManager 按 Google AC 串改写）、
  IronSource / Chartboost（自读 TCF）。
- **收不到**：Meta（要自己读 AC 串转交，MAX 不代劳）、AppsFlyer（要开 `enableTCFDataCollection`；
  原包直接写死「已同意」）。
...
1. **照原包的四条，逐条确认**（默认全部照原包，即 owner 2026-09-11 的选择）：
   a. Firebase 在 `onCreate` 写死 GRANTED，每次冷启约 5.5 秒窗口；
   b. 分析 / 归因先于同意启动，用户做选择前已记安装与首次打开；
   c. AppsFlyer 国家表照抄（希腊 `EL` / 英国 `UK` 落进非 GDPR 分支）；
   d. 首启表单不勾「不同意」（AdMob 后台配置）。
...
> 文档维护：Claude Opus 4.8（2026-09-11 建档）
